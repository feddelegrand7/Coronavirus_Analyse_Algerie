...
crn_alg_lg_f <- crn_alg_lg %>% filter(nombre > 0)
...
p1 <- ggplot(crn_alg_lg_f, aes(age)) +
  geom_bar() +
...
p2 <- ggplot(crn_alg_lg_f, aes(age)) +
  geom_bar(aes(y = ..prop.., group = 1)) +
  labs(title = "Distribution des cas de Covid-19 en Algérie selon l'Age au 30/03/2020", 
    y = "proportion", x = "", subtitle = "Fait par @moh_fodil", caption = "source des données: https://api.healthdz.com/covid19/now?pretty") +
  ggthemes::theme_pander()


ggsave(plot = p1, "age_dist.png", h = 14/3, w = 22/3, type = "cairo-png")
ggsave(plot = p2, "age_dist_prop.png", h = 14/3, w = 22/3, type = "cairo-png")
